...
original_start <- files %>%
  map(read_csv) %>%    # read in all the files individually, using
                       # the function read_csv() from the readr package
  reduce(rbind) 

original_start$timeid = substr(original_start$Image, 1, 11)
original_start$frame = parse_number(substr(original_start$Image, 20, 28))

# if fixed in the program, can delete this
original_start$TimeR = original_start$TimeR - original_start$Time # right eye segmented after left but time was not reset so subtracting that time here to get true right time 

# formatting into long format
# center x coordinate for right and left eye
centerx = original_start %>% select(Image, centerx, centerxR) %>% pivot_longer(cols = c("centerx", "centerxR"), names_to ="eye", values_to = "centerx") %>% mutate(eye = if_else(eye == "centerx","Left", "Right"))

# center y coordinate for right and left eye
centery = original_start %>% select(Image, centery, centeryR, frame, timeid) %>% pivot_longer(cols = c("centery", "centeryR"), names_to ="eye", values_to = "centery") %>% mutate(eye = if_else(eye == "centery","Left", "Right"))

# time for right and left eye
time = original_start %>% select(Image, Time, TimeR) %>% pivot_longer(cols = c("Time", "TimeR"), names_to ="eye", values_to = "time") %>% mutate(eye = if_else(eye == "Time","Left", "Right"))

# major axis size right and left eye
major_axis = original_start %>% select(Image, major_axis, major_axisR) %>% pivot_longer(cols = c("major_axis", "major_axisR"), names_to ="eye", values_to = "major_axis") %>% mutate(eye = if_else(eye == "major_axis","Left", "Right"))

# minor axis size right and left eye
minor_axis = original_start %>% select(Image, minor_axis, minor_axisR) %>% pivot_longer(cols = c("minor_axis", "minor_axisR"), names_to ="eye", values_to = "minor_axis") %>% mutate(eye = if_else(eye == "minor_axis","Left", "Right"))

# eccentricity size right and left eye
eccentricity = original_start %>% select(Image, eccentricity, eccentricityR) %>% pivot_longer(cols = c("eccentricity", "eccentricityR"), names_to ="eye", values_to = "eccentricity") %>% mutate(eye = if_else(eye == "eccentricity","Left", "Right"))
...
```{r}
original = original %>% select(-percent_change)

original = original %>% select(-initial)
...
pupils = read_csv(here::here("data", "pupils.csv"))
...
* total 90 unique (all jumps also in poor acf.)

* Exact count of how many required cleaning? (TBD: 40-50)

# Flagging participants who may need to be cleaned  manually 

## Calculating ACF and N missing for each particpant - time point prior to cleaning 

```{r, eval = FALSE}

## probably want to check between 3 and 6 standard deviations away 

## specify nrow of matrix

# nrow = len(timeid)

participant_id = original$timeid %>% unique()

storage_matrix = matrix(NA, nrow = length(participant_id)*2, ncol = 4)
...
for (iter_eye in 1:2){
  for (iter1 in 1:length(participant_id)){
    count = count + 1
    left_eye = original %>% filter(timeid == participant_id[iter1], eye == eyes[iter_eye]) #creating temporary data frame
    left_eye = left_eye[complete.cases(left_eye),]
  #
    if (nrow(left_eye) > 1){
      ## taking complete cases from temp data
  
    acf_no_clean = round(acf(left_eye$major_axis, pl = FALSE)$acf[2],4)
  
    n_obs = nrow(left_eye) # number of data points in original
  
    storage_matrix[count, ] = c(participant_id[iter1], as.numeric(acf_no_clean), n_obs, eyes[iter_eye])
    }
  
    if (nrow(left_eye)<1){
        storage_matrix[iter1, ] = c(participant_id[count], "NA", nrow(left_eye), eyes[iter_eye])
    }
  }
}
...
storage_matrix = rename(storage_matrix, acf_original = V2)
storage_matrix = rename(storage_matrix, completed_obs = V3)
storage_matrix = rename(storage_matrix, eyes = V4)


storage_matrix$acf_original = as.numeric(storage_matrix$acf_original)
...
#   for (iter1 in 1:length(participant_id)){
#     count = count + 1
#     left_eye = pupils %>% filter(timeid == participant_id[iter1], eye == eyes[iter_eye]) #creating temporary data frame
#     left_eye = left_eye[complete.cases(left_eye),]
#   #
#     if (nrow(left_eye) > 1){
#       ## taking complete cases from temp data
...
## Flagging those who have acf below 0.7

```{r, eval = FALSE}
acf_check = storage_matrix %>% filter(acf_original <=0.7) %>% select(timeid) %>% pull()

acf_check %>% length()
```

## Flagging those who little or no predicted pupil sizes (below 400)

```{r, eval = FALSE}
no_obs_check = original %>% group_by(timeid, eye) %>% mutate(completed_obs = n()) %>% ungroup() %>%  filter(completed_obs <400) %>% select(timeid)  %>% unique() %>% pull()

no_obs_check

```

## Flagging those that had lots of big jumps 


```{R, eval = FALSE}
original = original %>% group_by(timeid, eye) %>% mutate(lag_major = lag(major_axis), jump = major_axis - lag_major) %>% ungroup()


jump_check  = original %>% filter(abs(jump) > 10) %>% group_by(timeid, eye) %>% mutate(jump_count = n()) %>% ungroup() %>% select(timeid, eye, jump_count) %>% unique() %>% filter(jump_count>=50) %>%  select(timeid) %>% unique() %>% pull()
...
# Plots of potentially messy graphs


# ACF Messy

```{r, eval = FALSE}
for (i in 1:length(acf_check)){
x = original %>% filter(timeid == acf_check[i], eye == "Left") %>% select(major_axis) %>% pull()
x =x[!is.na(x)]
y = acf(x, pl = FALSE)$acf[2]

plot_major = ggplot(original %>% filter(timeid == acf_check[i], eye == "Left"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + labs(title = paste("Original : ACF = ",y),
              subtitle = paste("N = ",length(x)))

plot_major_cleanQ = ggplot(pupils %>% filter(timeid ==acf_check[i], eye == "Left"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + geom_line(aes(y = pupils %>% filter(timeid == acf_check[i], eye == "Left") %>% select(predicted) %>% pull()), color = "red")  

x = original %>% filter(timeid == acf_check[i], eye == "Right") %>% select(major_axis) %>% pull()
x =x[!is.na(x)]
y = acf(x, pl = FALSE)$acf[2]

plot_major_right = ggplot(original %>% filter(timeid == acf_check[i], eye == "Right"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + labs(title = paste("Original : ACF = ",y),
              subtitle = paste("N = ",length(x)))

plot_major_cleanQ_right = ggplot(pupils %>% filter(timeid ==acf_check[i], eye == "Right"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + geom_line(aes(y = pupils %>% filter(timeid == acf_check[i], eye == "Right") %>% select(predicted) %>% pull()), color = "red")  

plot = ggarrange(plot_major, plot_major_cleanQ, plot_major_right, plot_major_cleanQ_right)


print(annotate_figure(plot, top = text_grob(acf_check[i],
...
```{r, eval = FALSE}
for (i in 1:length(no_obs_check)){

x = original %>% filter(timeid == no_obs_check[i], eye == "Left") %>% select(major_axis) %>% pull()
x =x[!is.na(x)]
y = acf(x, pl = FALSE)$acf[2]

plot_major = ggplot(original %>% filter(timeid == no_obs_check[i], eye == "Left"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") 
         

print(annotate_figure(plot_major, top = text_grob(no_obs_check[i], 
               color = "red", face = "bold", size = 14)))
}
```
...
```{r, eval = FALSE}
for (i in 1:length(jump_check)){


x = original %>% filter(timeid == jump_check[i], eye == "Left") %>% select(major_axis) %>% pull()
x =x[!is.na(x)]
y = round(acf(x, pl = FALSE)$acf[2],3)

plot_major = ggplot(original %>% filter(timeid == jump_check[i], eye == "Left"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + labs(title = paste("Left Eye: ACF = ",y),
              subtitle = paste("N = ",length(x)))

plot_major_cleanQ = ggplot(pupils %>% filter(timeid ==jump_check[i], eye == "Left"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + geom_line(aes(y = pupils %>% filter(timeid == jump_check[i], eye == "Left") %>% select(predicted) %>% pull()), color = "red")  + ggtitle("Left Cleaned")

x = original %>% filter(timeid == jump_check[i], eye == "Right") %>% select(major_axis) %>% pull()
x =x[!is.na(x)]
y = round(acf(x, pl = FALSE)$acf[2],3)

plot_major_right = ggplot(original %>% filter(timeid == jump_check[i], eye == "Right"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + labs(title = paste("Right Eye: ACF = ",y),
              subtitle = paste("N = ",length(x)))

plot_major_cleanQ_right = ggplot(pupils %>% filter(timeid ==jump_check[i], eye == "Right"), aes(x = frame, y = major_axis)) + geom_point() + ylab("Major Axis Size") + geom_line(aes(y = pupils %>% filter(timeid == jump_check[i], eye == "Right") %>% select(predicted) %>% pull()), color = "red") + ggtitle("Right Cleaned")  

plot = ggarrange(plot_major, plot_major_cleanQ, plot_major_right, plot_major_cleanQ_right)


print(annotate_figure(plot, top = text_grob(jump_check[i],
               color = "red", face = "bold", size = 14)))
}
```
...
```{r}
filename = "pupils_full_clean.csv"

write_csv(pupils, file = here::here("data", filename))
