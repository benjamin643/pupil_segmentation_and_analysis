...
output = readRDS(here::here("data", "raw_pupil_size_predictions.rds")) %>%
  mutate(timeid = substr(Image, 1, 11),
         frame = parse_number(substr(Image, 20, 28))) %>%
  select(-contains("minor_axis"), -contains("Time"), -eccentricity)
...
# write_csv(pupils_fin, file = here::here("data", filename))
