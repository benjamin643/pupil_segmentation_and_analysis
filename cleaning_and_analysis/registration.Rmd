...
pupils = read_csv(here::here("data", "pupils_full_clean.csv"))
# 
...
# 
# ## 2nd der
# new_data$max2 = der_vec_max2
# 
# new_data$min2 = der_vec_min2

# new_data$extremum2 = new_data$max2 + new_data$min2
# 
# new_data$extremum2 = factor(new_data$extremum2)
# 
# 
...
```{R, eval = FALSE}
temp_graph = storage_matrix %>% filter(timeid == "001-111post", eye == "Right")

starting = temp_graph %>% filter(centered == "0") %>% select(frame) %>% pull()

plot1 = ggplot(temp_graph, aes(x = frame, y = percent_change )) + geom_line() + geom_vline(aes(xintercept=141),linetype="dotted") + geom_vline(aes(xintercept= 75 )) +ylab("Predicted Major Axis Size") + ggtitle("A") + theme(plot.title = element_text(size = 20, face = "bold")) +
  theme(plot.title = element_text(hjust = 0.5))

plot2 = ggplot(temp_graph, aes(x = centered, y = percent_change )) + geom_line()  +ylab("Predicted Major Axis Size") + geom_vline(aes(xintercept=0),linetype="dotted") + ggtitle("B") + theme(plot.title = element_text(size = 20, face = "bold")) +
  theme(plot.title = element_text(hjust = 0.5))

plot3 = ggplot(storage_matrix %>% filter(centered >= 0, eye == "Right"), aes(x = centered, y = percent_change, fill = timeid)) + geom_line(alpha = 0.2)  + theme(legend.position = "none") + ggtitle("C") + ylab("Percent Change from Baseline") + theme(plot.title = element_text(size = 20, face = "bold")) +
  theme(plot.title = element_text(hjust = 0.5))
...
```{r}
filename = "pupils_clean_register.csv"


write.csv(as.data.frame(storage_matrix), file = here::here("data", filename))
